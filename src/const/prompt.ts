...
- 仅输出 JSON String，不要包含任何解释、外链、引用标签或代码块标记（例如 \`\`\`）。
...
## 附加上下文
- 当前的系统日期为：${new Date().toLocaleDateString()}
`;
...
- 如果用户提供的文本和目标语言相同，请直接返回原文本。
  - 比如用户提供的文本是中文，目标语言也是中文，无需进行翻译，直接返回用户提供的文本。
  - 比如用户提供的文本是韩文，目标语言也是韩文，无需进行翻译，直接返回用户提供的文本。
